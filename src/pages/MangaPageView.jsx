...
const MangaReader = () => {
  ...
  const sendHeartbeat = useCallback(
    async (isFinished = false) => {
      ...
      if (!token || !mangaData || lockStatus.isLocked || document.hidden)
        return;
      try {
        await axios.post(
          `${API_URL}/api/analytics/heartbeat`,
          {
            mangaId: id,
  ...
  const syncLibraryProgress = useCallback(
    async (title, total) => {
      const token = localStorage.getItem("token");
      ...
      const isSeriesFinished = mangaData.status?.toLowerCase() === "completed";
      const finalStatus =
        isLastChapter && isSeriesFinished ? "Completed" : "Reading";

      try {
        await axios.post(
          `${API_URL}/api/library/update`,
          {
            mangaId: id,
            mangaTitle: title,
        ...
        if (isLastChapter && isSeriesFinished) {
  ...
  if (loading || !mangaData)
    return (
      <div className="min-h-screen bg-[#050505] flex items-center justify-center">
        <div className="relative">
          <div
            className={`w-16 h-16 border-4 border-white/5 rounded-full animate-spin`}
    ...
    <div className="min-h-screen bg-[#050505] text-white font-sans selection:bg-white/20 overflow-x-hidden">
      {!readingMode && (
        <header className="relative bg-[#0f0f10] pt-24 md:pt-32 pb-12 md:pb-16 px-4 border-b border-white/5 overflow-hidden">
          <div
            className={`absolute -top-24 -left-24 w-64 md:w-96 h-64 md:h-96 ${glowBg} blur-[80px] rounded-full pointer-events-none`}
          />
          ...
          <div className="absolute top-6 left-0 right-0 px-4 md:px-8 flex justify-between items-center z-20">
            <button
              ...
              className="flex items-center gap-2 px-3 py-2 bg-white/5 border border-white/10 rounded-full text-[10px] font-bold uppercase tracking-widest hover:bg-white/10 transition-all group"
            >
              <ArrowLeft
                size={14}
            ...
            <div
              className={`flex items-center gap-2 px-3 py-2 bg-white/5 border border-white/10 rounded-full text-[9px] font-bold uppercase tracking-widest 
          ...
          <div className="max-w-5xl mx-auto flex flex-col md:flex-row items-center gap-6 md:gap-10 relative z-10">
            ...
            <div className="relative group shrink-0 w-44 md:w-80">
              <img
                ...
                alt="Cover"
                className="rounded-xl md:rounded-2xl shadow-2xl border border-white/10 w-full object-cover aspect-[3/4]"
              />
              {isFullyCompleted && (
                <div className="absolute -top-3 -right-3 bg-green-500 text-white p-2 md:p-3 rounded-full shadow-xl animate-bounce">
                  <PartyPopper size={18} />
                </div>
              )}
            </div>
            ...
            <div className="flex-1 text-center md:text-left pt-2 md:pt-4 w-full">
              ...
              <div className="flex flex-wrap gap-2 justify-center md:justify-start mb-4">
                {mangaData.tags?.slice(0, 3).map((tag, index) => (
                  <span
                    key={index}
                    className="text-[8px] font-bold uppercase tracking-widest px-2 py-1 rounded-md bg-white/5 border border-white/10 text-slate-400"
                  >
                    {tag}
                  </span>
                ))}
              ...
              <p
                className={`${accentText} text-xs md:text-base font-bold uppercase tracking-[0.3em] mb-4`}
              >
                {lockStatus.isLocked
                  ? "ACCESS RESTRICTED"
                  : isFullyCompleted
                    ...
                    : parseInt(chapterNum) >= mangaData.TotalChapter
                      ? "END OF CURRENT RELEASE" // Better for ongoing series
      ...
      <div className="max-w-4xl mx-auto py-6 md:py-10 px-0 sm:px-4">
        {lockStatus.isLocked ? (
          <div className="mx-4 py-16 text-center flex flex-col items-center bg-white/[0.02] rounded-[2rem] border border-white/5 p-8">
            ...
            <h2 className="text-2xl font-bold uppercase italic tracking-tighter mb-4">
              Transmission Blocked
            </h2>
            <p className="text-slate-400 mb-8 max-w-xs text-xs leading-relaxed">
              {lockStatus.reason}
            </p>
            <div className="flex flex-col gap-3 w-full max-w-xs">
              <button
                onClick={() => navigate("/shop")}
                className="w-full py-4 bg-white text-black font-bold uppercase text-[10px] tracking-widest rounded-xl flex items-center justify-center gap-2"
              >
                <ShoppingBag size={14} /> Upgrade Protocol
              </button>
              <button
                onClick={() => navigate(-1)}
                className="w-full py-4 bg-white/5 border border-white/10 font-bold uppercase text-[10px] tracking-widest rounded-xl flex items-center justify-center gap-2"
              >
