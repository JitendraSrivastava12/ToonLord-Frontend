    ...
    <div className="fixed inset-0 z-[120] flex items-center justify-center p-4">
      ...
      <div className="relative bg-white w-full max-w-sm rounded-[2.5rem] p-8 shadow-2xl animate-in zoom-in-95 duration-200 border border-gray-100">
        <div className="flex flex-col items-center text-center space-y-4">
          <div
            className={`p-4 rounded-3xl ${variant === "danger" ? "bg-red-50 text-red-600" : "bg-yellow-50 text-yellow-600"}`}
          >
        ...
        <div className="p-8 pb-0 text-center relative shrink-0">
          ...
          <div className="flex flex-col items-center gap-4">
            <div className="relative">
              ...
              />
              <div
                className={`absolute -bottom-1 -right-1 w-7 h-7 rounded-full border-4 border-white shadow-sm ${
                  user.status === "active"
                    ? "bg-green-500"
                    : user.status === "banned"
                      ? "bg-red-500"
        ...
        <div className="p-8 pt-10 space-y-8 overflow-y-auto no-scrollbar">
          <div className="space-y-4">
            ...
            {user.status === "suspended" && (
              <div className="grid grid-cols-1 gap-3">
                {/* Status Alert - Matches Active Padding */}
                <div className="p-5 bg-yellow-50/50 border border-yellow-100 rounded-[2rem] flex gap-4 text-xs text-yellow-800 font-bold leading-relaxed">
                  <ShieldAlert className="text-yellow-600 shrink-0" size={20} />
                  <div>
                    Restricted due to <strong>{user.reports} reports</strong>.
                    <p className="mt-1 font-medium opacity-80">
                      User is currently in "read-only" mode.
                    </p>
                  </div>
                </div>

                {/* Primary Action: Lift Restriction */}
                <button
                  onClick={() => onUpdateStatus(user.id, "active")}
                  className="flex items-center gap-4 p-5 bg-gray-900 text-white rounded-[2rem] hover:bg-black transition-all group"
                >
                ...
                {/* Added Ban Button: To allow escalating from Suspension to Ban directly */}
                <button
                  onClick={() => onUpdateStatus(user.id, "banned")}
                  className="flex items-center gap-4 p-5 bg-white border border-gray-100 rounded-[2rem] hover:border-red-400 hover:bg-red-50 transition-all group"
                >
...
const UserManagement = () => {
  ...
  const handleUpdateStatus = async (userId, newStatus) => {
    try {
      ...
      showAlert(
        `Security level updated: ${newStatus.toUpperCase()}`,
        "success",
      );
    } catch (err) {
  ...
  const handleExecuteDelete = async () => {
    ...
    try {
      ...
      showAlert(
        err.response?.data?.message || "Purge failed: Server error",
        "error",
      );
    }
  };
  ...
  const handleExportList = () => {
    ...
    link.setAttribute(
      "download",
      `ToonLord_Users_${activeFilter.replace(" ", "_")}_${date}.csv`,
    );
    ...
    showAlert(
      `Exported ${filteredUsers.length} records successfully.`,
      "success",
    );
  };
      ...
      <div className="max-w-7xl mx-auto space-y-6 sm:space-y-10">
        ...
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-end gap-6">
          <div className="space-y-2">
            ...
            <p className="text-gray-400 text-sm font-bold">
              Control access and safety for ToonLord
            </p>
          </div>
          <button
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-8">
          {stats.map((stat, i) => (
            <div
              key={i}
              className="bg-white p-6 sm:p-8 rounded-[2.5rem] border border-gray-100 shadow-sm flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4"
            >
              ...
              <div
                className={`p-4 rounded-[1.5rem] ${stat.bg} ${stat.text} shadow-sm`}
              >
                {stat.icon}
              </div>
            </div>
              ...
              <tbody className="divide-y divide-gray-50">
                {filteredUsers.map((user) => (
                  ...
                  >
                    <td className="px-10 py-6">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-gray-400 font-bold mt-1 tracking-tight">
                            {user.email}
                          </p>
                        </div>
                      </div>
                    ...
                    <td className="px-10 py-6">
                      <span
                        className={`text-[9px] font-black uppercase px-3 py-1.5 rounded-xl border tracking-widest shadow-sm ${
                          user.status === "active"
                    ...
                    <td className="px-10 py-6 text-xs font-bold">
                      ...
                      <p className="text-gray-400 text-[10px] mt-0.5 uppercase">
                        {user.role}
                      </p>
                    </td>
                    <td className="px-10 py-6">
                      <span
                        className={`text-sm font-black ${user.reports > 5 ? "text-red-500" : "text-gray-300"}`}
                      >
                        {user.reports}
                      </span>
                    </td>
