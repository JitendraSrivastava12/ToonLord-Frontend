...
function NavBar({ setIsSidebarOpen }) {
  ...
  // --- NOTIFICATION LOGIC (STRICT FILTER) ---
  ...
  const getNotificationIcon = (category, type) => {
    if (category === 'system') return <ShieldCheck size={14} className="text-red-500" />;
            ...
            {showSearchDropdown && suggestions.length > 0 && (
              ...
              >
                {suggestions.map((manga) => (
                  <div key={manga._id} onClick={() => { navigate(`/manga/${manga._id}`); setShowSearchDropdown(false); setSearchQuery(""); }}
                  ...
                  >
                    ...
                    <div className="flex flex-col">
                      ...
                      <span className="text-[9px] px-2 py-0.5 rounded-full bg-blue-500/10 text-blue-500 font-bold uppercase w-fit mt-1">{manga.status || 'Ongoing'}</span>
                    </div>
                  </div>
                ))}
              </motion.div>
              ...
              {isBellOpen && (
                ...
                >
                  <div className="p-4 md:p-6 border-b border-white/5 flex justify-between items-center bg-white/[0.02]">
                    <h3 className={`text-[9px] md:text-[10px] font-black uppercase tracking-[0.3em] ${themeStyles.subtext}`}>Registry: {allowedCategories.join(' & ')}</h3>
                    {unreadCount > 0 && (
                      <button onClick={handleMarkAllRead} className="group flex items-center gap-2 px-3 py-1.5 rounded-full bg-blue-500/10 border border-blue-500/20 hover:bg-blue-600 transition-all">
                        ...
                        <span className="text-[8px] md:text-[9px] font-black text-blue-500 group-hover:text-white uppercase">Clear</span>
                      </button>
                    )}
                  </div>
                  ...
                  <div className="max-h-[300px] md:max-h-[380px] overflow-y-auto custom-scrollbar">
                    {previewNotifications.length > 0 ? (
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            <p className={`text-[12px] md:text-[13px] leading-snug mb-1 ${themeStyles.text} ${!notif.isRead ? 'font-medium' : 'opacity-60'}`}>
                              <span className={`font-black italic mr-1 ${notif.category === 'system' ? 'text-red-500' : 'text-blue-500'}`}>
                      ...
                      <div className="py-12 md:py-16 flex flex-col items-center justify-center opacity-20">
                        ...
                        <p className="text-[10px] font-black uppercase tracking-[0.3em]">No Priority Logs</p>
                      </div>
                    )}
                  </div>
