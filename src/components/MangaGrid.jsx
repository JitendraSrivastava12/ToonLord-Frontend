...
import { Star, Zap, Loader2, ShieldAlert, Baby, Orbit } from 'lucide-react';
...
const MangaGrid = ({ category }) => {
  ...
  const mangaDisplay = useMemo(() => {
    ...
    if (category === 'trending') filtered = [...filtered].sort((a,b) => (b.views||0)-(a.views||0)).slice(0,12);
    else if (category === 'top-rated') filtered = [...filtered].sort((a,b)=>(b.rating||0)-(a.rating||0)).slice(0,12);
    return filtered;
          ...
          >
            ...
            >
              ...
              <div className="relative aspect-[3/4.2] overflow-hidden rounded-t-2xl">
                ...
                {/* PREMIUM / FREE BADGE */}
                {manga.isPremium ? (
                  <div className={`absolute top-3 left-3 px-3 py-1 rounded-full text-xs flex items-center gap-1.5 ${
                    isRedMode ? 'bg-red-600 text-white' : 'bg-[var(--accent)] text-white'
                  }`}>
                    <Zap size={12} /> VIP
                  </div>
                ) : (
                  <div className="absolute top-3 left-3 px-3 py-1 bg-black/60 backdrop-blur-md text-white text-xs rounded-full uppercase tracking-wider border border-white/5">
                    Free
                  </div>
                )}
              ...
              <div className="px-4 py-3 flex flex-col gap-1">
                <h3 className="text-lg font-black line-clamp-2 text-[var(--text-main)] group-hover:text-[var(--accent)]">
                  {manga.title}
                </h3>
                <div className="text-sm opacity-60 flex justify-between">
      ...
      {mangaDisplay.length===0 && (
        <motion.div initial={{opacity:0}} animate={{opacity:1}} className="col-span-full py-40 text-center space-y-6">
          ...
          <div className="space-y-1">
            ...
            <p className="text-[10px] font-bold text-[var(--text-dim)] uppercase tracking-widest">No transmissions found in this sector.</p>
          </div>
        </motion.div>
      )}
    </div>
